...
function MyAccountTab({ user, logout }) {
  return (
...
function PersonalInfoTab({ user, setToast }) {
...
function ThemeTab({ user, setToast }) {
  const { updateUser } = useAuth();
      ...
      <div className="grid grid-cols-3 gap-3">
        {OPTIONS.map((o) => (
          <button
            ...
            className={`rounded-xl border p-4 text-center transition disabled:opacity-50 ${current === o.key
              ? "border-seal bg-seal/10"
              : "border-black/10 dark:border-line bg-white"
              }`}
          >
...
function NotificationsTab({ user, setToast }) {
  const { updateUser } = useAuth();
...
function PaymentAccountsTab({ setToast }) {
  ...
  return (
    ...
    >
      ...
      <input
        ...
        value={value}
        onChange={onChange}
        className={`w-full mt-1.5 border rounded-lg px-3 py-2 text-sm bg-white dark:bg-panel text-ink dark:text-paper ${error
          ? "border-alarm"
          : "border-black/10 dark:border-line"
          }`}
      />
