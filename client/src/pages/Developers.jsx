  ...
  return <div className="min-h-screen bg-[#f6f7f4] text-black">
    ...
    <main className="mx-auto max-w-6xl px-5 py-10 sm:px-8">
      ...
      {section === 'Documentation' && <article className="space-y-6 rounded-2xl border border-black/15 bg-white p-6 sm:p-8">
        ...
        <h3 className="font-semibold">POST /verify</h3><p>Send <code>provider</code>, <code>reference</code>, and the merchant's expected amount and receiver details. Supported providers: {config?.providers.join(', ') || 'loading...'}. This endpoint accepts references and supported receipt links, not image OCR uploads.</p>
        <pre className="overflow-x-auto rounded-xl bg-[#151515] p-5 text-sm text-white">{`curl -X POST '${base}/verify' \\\n  -H "Authorization: Bearer $DU_API_KEY" \\\n  -H 'Content-Type: application/json' \\\n  -H 'Idempotency-Key: order_12345_attempt_1' \\\n  -d '{"provider":"CBE","reference":"https://mbreciept.cbe.com.et/v2-example-token","expectedAmount":250,"receiverAccountNumber":"100012345678","receiverAccountHolderName":"DU Verifay"}'`}</pre>
        <p><code>receiverAccountNumber</code> and <code>receiverAccountHolderName</code> activate the merchant receiver check; <code>expectedAmount</code> activates the amount check. <code>returnDetails: true</code> includes the provider-specific receipt payload for manual review. <code>phoneNumber</code> is required for CBE Birr. Abyssinia needs <code>receiverAccountNumber</code> so its last five digits can be checked. CBE accepts only the complete official mobile-banking receipt link, not FT references, raw tokens, or USSD screenshots.</p>
        <h3 className="font-semibold">Responses and payment acceptance</h3><pre className="overflow-x-auto rounded-xl bg-[#151515] p-5 text-sm text-white">{JSON.stringify({ requestId: '...', status: 'VALID', charged: 1, currency: 'DU_PT', provider: 'CBE', verification: { badge: 'green', receiptFound: true, amountMatched: true, receiverMatched: true, duplicate: false } }, null, 2)}</pre>
        <p><code>VALID</code> is green: receipt found and all supplied merchant checks passed. Yellow means amount or receiver mismatch. Red means unconfirmed or duplicate. Black means provider/OCR/site error: show “Please try again”; for unreadable receipts, offer manual reference entry. If no merchant fields are supplied, full provider detail is returned instead of claiming a green merchant match.</p>
