// components/BrokenByDesign.jsx
//
// "Broken by design" glass-shard hero, adapted for DU Verifay.
//
// Converted from the original TSX to plain JSX (this project is
// JavaScript + Vite + Tailwind; no TypeScript / shadcn needed —
// the component is fully self-contained with its own scoped CSS).
//
// Assets are served locally from /assets/broken-by-design/ (see
// client/public). The CDN remains as a fallback via props.
// Sound is OFF by default to match the app's minimal character.

import { useEffect, useMemo, useRef, useState } from "react";

/* Inlined so this file is self-contained. Rendered as a single
   <style> tag; all classes carry the "bbd2-" prefix so nothing
...
/* ------------------------------------------------------------------ */
/*  Data: desktop / mobile piece sets, sprite atlases, crack lines.   */
/* ------------------------------------------------------------------ */
...
const MOBILE = [
  ...
  { id: 'mobile-04c', x: 42.556, y: 56.508, w: 47.831, h: 13.503, cx: 66.0, cy: 56.508, ring: 0 },
...
export default function BrokenByDesign({
  assetsBase = '/assets/broken-by-design',
  title = 'DU VERIFAY',
  height = '100%',
  sound = false,
  interactive = true,
  ...
  return (
    <>
      ...
      <section
        ref={rootRef}
        className={`bbd2 ${portrait ? 'bbd2--portrait' : ''} ${className}`}
        style={{ height }}
        aria-label={title}
      >
          ...
          {ready && (
            <>
              ...
              <div className="bbd2-pane" aria-hidden="true">
                {pieces.map((p, i) => {
                  ...
                  return (
                    ...
                    >
                      <div className="bbd2-inlay">
                        <div
                          className="bbd2-glassimg"
                          style={{
                            backgroundImage: `url(${atlasUrl})`,
                            ...spriteStyle(setKey, p.id),
                          }}
                        />
                        <div className="bbd2-slice">
                          {titleNode}
                        </div>
                      </div>
              ...
              <button
                ...
                aria-label={soundOn ? 'Mute crack sounds' : 'Enable crack sounds'}
                onClick={() => setSoundOn((s) => !s)}
              >
                <img src={`${assetsBase}/${soundOn ? 'sound-on.png' : 'sound-off.png'}`} alt="" />
              </button>
            </>
          )}
        </div>
